...
function renderLine(line: string, i: number) {
  ...
  if (line.startsWith("> ")) return <blockquote key={i} className="border-l-4 border-amber-500 pl-4 italic my-4 bg-amber-50 dark:bg-amber-950/20 py-2 rounded-r text-gray-700 dark:text-gray-100">{line.slice(2)}</blockquote>
  if (line.startsWith("---")) return <hr key={i} className="border-gray-200 dark:border-gray-700 my-8" />
  if (line.trim() === "") return <div key={i} className="h-2" />
  return <p key={i} className="text-gray-700 dark:text-gray-100 leading-relaxed" dangerouslySetInnerHTML={{__html: boldify(line)}} />
}
  ...
  const jsx = (
    <div key={startIndex} className="overflow-x-auto my-6">
      <table className="w-full text-sm border-collapse">
        ...
        <tbody>
          {rows.map((row, j) => (
            <tr key={j} className={j % 2 === 0 ? "bg-white dark:bg-gray-800" : "bg-gray-50 dark:bg-gray-750"}>
              {row.map((cell, k) => <td key={k} className="px-4 py-3 text-gray-900 dark:text-white border border-gray-200 dark:border-gray-500">{cell}</td>)}
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-12">
      <div className="mb-10">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-300">{data.date} · {data.author} · {data.readTime}</p>
      </div>
