...
function renderLine(line: string, i: number) {
  ...
  if (line.startsWith("- ")) return <li key={i} className="ml-6 text-gray-300 list-disc leading-relaxed">{line.slice(2)}</li>
  if (line.startsWith("> ")) return <blockquote key={i} className="border-l-4 border-amber-500 pl-4 text-gray-400 italic my-4 bg-amber-950/20 py-2 rounded-r">{line.slice(2)}</blockquote>
      ...
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="bg-amber-900/40">
            {headers.map((h, j) => <th key={j} className="px-4 py-3 text-left text-amber-400 font-semibold border border-gray-700">{h}</th>)}
          </tr>
        </thead>
        <tbody>
          {rows.map((row, j) => (
            <tr key={j} className={j % 2 === 0 ? "bg-gray-900/40" : "bg-gray-800/20"}>
              {row.map((cell, k) => <td key={k} className="px-4 py-3 text-gray-300 border border-gray-700">{cell}</td>)}
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-12">
      <div className="mb-10">
        ...
        <h1 className="text-3xl font-semibold mt-4 mb-3 text-white">{data.title}</h1>
